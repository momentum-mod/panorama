...
class Toast {
	...
	duration: number;
	customLayout: string;
	location: Array<'left' | 'center' | 'right'>;
	panel: GenericPanel;
	expiring: boolean;
...
class ToastManager {
	// { left: [], ...}
activeToasts = this.makeToastLocationsArray();
	// { left: [], ...}
queuedToasts = this.makeToastLocationsArray();
	// { left: $('#Left'), ...}
containers;
