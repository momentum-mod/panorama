...
class Toast {
	...
	constructor(args: ToastCreateArgs) {
...
class ToastManagerInternal {
	readonly activeToasts: Map<ToastLocation, Toast[]> = new Map(Enum.values(ToastLocation).map((loc) => [loc, []]));
	readonly queuedToasts: Map<ToastLocation, Toast[]> = new Map(Enum.values(ToastLocation).map((loc) => [loc, []]));
	readonly containers: ReadonlyMap<ToastLocation, Panel> = new Map([
		[ToastLocation.LEFT, $('#Left')],
		[ToastLocation.CENTER, $('#Center')],
		[ToastLocation.RIGHT, $('#Right')]
	]);
	readonly classes: ReadonlyMap<ToastLocation, string> = new Map([
	...
	constructor() {
		$.RegisterForUnhandledEvent('Toast_Show', (id, title, message, location, duration, icon, style) =>
			this.queueToast({ id, title, message, location, duration, style, icon })
		);
		$.RegisterForUnhandledEvent('Toast_ShowCustom', (id, layoutFile, location, duration, parameters) =>
			this.queueToast({ id, layoutFile, location, duration, parameters })
		);
	...
	queueToast(toastArgs: ToastCreateArgs) {
		if (!toastArgs) return;

		const toast = new Toast(toastArgs);

		const existingToast = this.activeToasts.get(toast.location).find((t: Toast) => t.isIdenticalTo(toast));
		if (existingToast) {
	...
	private onToastExpired(location: ToastLocation) {
		if (this.queuedToasts.get(location).length > 0) {
			this.createToast(this.queuedToasts.get(location)[0]);
