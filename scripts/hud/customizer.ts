...
// Sasha: had to change some types from Panel to GenericPanel
...
function loadComponentPanel(component: Pick<Component, 'position' | 'snaps'> | undefined, panel: GenericPanel) {
	if (component) {
...
function getNearestGridLine(axis: Axis, sizeFactor: number): Gridline {
	...
	const relativeOffset = isX
		? Math.max(
				0,
				Math.min(1920, (panels.virtual.actualxoffset + panels.virtual.actuallayoutwidth * sizeFactor) / scaleX)
			)
		: Math.max(
				0,
				Math.min(1080, (panels.virtual.actualyoffset + panels.virtual.actuallayoutheight * sizeFactor) / scaleY)
			);
