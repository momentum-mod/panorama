namespace Cgaz {
	...
	class Cgaz {
		...
		static primeZones = [];
		static primeFirstZoneLeft = this.initZonePanel(
			$.CreatePanel('Panel', this.primeContainer, 'PrimeFirstZoneLeft')
		);
		static primeFirstZoneRight = this.initZonePanel(
			$.CreatePanel('Panel', this.primeContainer, 'PrimeFirstZoneRight')
		);
		static primeSplitZone = this.initZonePanel($.CreatePanel('Panel', this.primeContainer, 'PrimeSplitZone'));
		static primeHighlightZone = this.initZonePanel(
			$.CreatePanel('Panel', this.primeContainer, 'PrimeHighlightZone')
		);
		static primeArrow = $('#PrimeArrow');
		static primeArrowIcon = $('#PrimeArrowIcon');
		static primeAccel: number;
		...
		static pitchStat = $<Label>('#PitchStat');
		static yawStat = $<Label>('#YawStat');
		...
		static primeAngles: number[] = [];
		static snapGainRange: number[] = []; // stored as [min, max]
		static snapAngles: number[] = [];
		static snapAccel = 0;
		static bShouldUpdateStyles = false;

		static projection: 0 | 1 | 2;
		static hudFov: number;

		static accelEnable: boolean;
		...
		static primeEnable: boolean;
		static primeTruenessMode: int32;
		static primeShowInactive: boolean;
		static primeLockOneLine: boolean;
		...
		static compassArrowSize: float;
		static compassTickSize: float;
		static compassOffset: float;
		static compassPitchEnable: boolean;
		static compassPitchTarget: number[];
		...
		static onLoad() {
			if (
				_.Web.GamemodeCategories.get(_.Web.GamemodeCategory.DEFRAG).includes(GameModeAPI.GetCurrentGameMode())
			) {
		...
		static onUpdate() {
			...
			const bSnapShift =
				!_.Util.Maths.approxEquals(Math.abs(forwardMove), Math.abs(rightMove), 0.01) &&
				!(phyMode && bIsFalling);
			...
			if (this.snapEnable && this.snapAccel) {
				const snapOffset = _.Util.Maths.remapAngle(
				...
				if (speed >= this.snapMinSpeed) {
					...
					const targetAngle = this.findFastAngle(
						dropSpeed,
						MAX_GROUND_SPEED,
						MAX_GROUND_SPEED * tickInterval
					);
			...
			if (this.primeEnable) {
				...
				if (speed > this.primeMinSpeed) {
					...
					const primeSightAccel =
						this.primeTruenessMode & TruenessMode.CPM_TURN ? primeMaxAccel : this.snapAccel;
					...
					if (this.primeArrowEnable) {
						if (_.Util.Maths.getSizeSquared2D(wishDir) > 0) {
							...
							const leftEdge =
								_.Util.Maths.mapAngleToScreenDist(
									arrowAngle,
									this.hFov,
									this.screenX,
									this.scale,
			...
			if (this.compassPitchEnable) {
				...
				for (let i = 0; i < pitchLines?.length; ++i) {
					...
					const pitchDeltaPx = _.Util.Maths.mapAngleToScreenDist(
		...
		static findSnapAngles(snapAccel: number): number[] {
			...
			for (let i = 0; i < points; ++i)
				angles.push(
					-breakPoints[i],
					_.Util.Maths.remapAngle(Math.PI * 0.5 - breakPoints[i]),
					_.Util.Maths.remapAngle(breakPoints[i] - Math.PI * 0.5)
				);

			return angles.sort((a, b) => a - b);
		}

		static findBreakPoints(accel: number, value: number, breakPoints: number[]) {
		...
		static updateZone(zone, left, right, offset, zoneClass, splitZone) {
			...
			zone.leftPx = _.Util.Maths.mapAngleToScreenDist(
				zone.leftAngle,
				this.hFov,
				this.screenX,
				this.scale,
			...
			zone.rightPx = _.Util.Maths.mapAngleToScreenDist(
				zone.rightAngle,
				this.hFov,
				this.screenX,
				this.scale,
			...
			if (wrap) {
				...
				splitZone.leftPx = _.Util.Maths.mapAngleToScreenDist(
				...
				splitZone.rightPx = _.Util.Maths.mapAngleToScreenDist(
		...
		static updatePrimeSight(viewDir, viewAngle, targetAngle, boundaryAngle, velAngle, wishDir, wishAngle) {
			...
			const inputMode =
				Math.round(_.Util.Maths.getSize2D(wishDir)) * (1 << Math.round(2 * Math.pow(cross, 2))) +
			...
			if (fillLeftZones || this.primeShowInactive) {
				this.primeFirstZoneLeft.rightPx = _.Util.Maths.mapAngleToScreenDist(
					_.Util.Maths.wrapToHalfPi(leftTarget - leftOffset),
					this.hFov,
					this.screenX,
					this.scale,
					this.projection
			...
			speedGain = this.findPrimeGain(
				this.primeFirstZoneLeft,
				velocity,
				_.Util.Maths.rotateVector2D(viewDir, 0.25 * Math.PI)
			);
			...
			if (fillRightZones || this.primeShowInactive) {
				this.primeFirstZoneRight.leftPx = _.Util.Maths.mapAngleToScreenDist(
					_.Util.Maths.wrapToHalfPi(rightTarget - rightOffset),
					this.hFov,
					this.screenX,
					this.scale,
					this.projection
		...
		static findPrimeGain(zone, velocity: Util.Maths.Vector, wishDir) {
		...
		static initZonePanel(panel: Panel) {
