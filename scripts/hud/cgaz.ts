import { Component,OnMapLoad } from 'util/component';
...
import { enhanceAlpha, rgbaStringLerp, rgbaStringToRgb } from 'util/color';
...
@Component
class CgazComponent implements OnMapLoad {
	...
	primeArrowIcon = $('#PrimeArrowIcon');
	primeAccel: number;

	compassArrow = $('#CompassArrow');
	compassArrowIcon = $('#CompassArrowIcon');
	...
	windicatorArrowIcon = $('#WindicatorArrowIcon');
	...
	onUpdate() {
		...
		const bHasAirControl = phyMode && approxEquals(wishAngle, viewAngle, 0.01) && bIsFalling;
		const bSnapShift = !approxEquals(Math.abs(forwardMove), Math.abs(rightMove), 0.01) && !(phyMode && bIsFalling);
		...
		if (this.accelEnable) {
			...
			if (speed >= this.accelMinSpeed && this.accelMirrorEnable) {
				...
				if (approxEquals(Math.abs(inputAngle), 0.25 * Math.PI, 0.01)) {
	...
	findSlowAngle(dropSpeed, dropSpeedSquared, speedSquared, maxSpeed) {
	...
	findFastAngle(dropSpeed, maxSpeed, maxAccel) {
		const threshold = maxSpeed - maxAccel;
		return Math.acos(dropSpeed < threshold ? 1 : threshold / dropSpeed);
	}

	findTurnAngle(speed, dropSpeed, maxAccel, fastCgazAngle) {
		const threshold = speed - dropSpeed;
		return Math.max(Math.acos(maxAccel < threshold ? 1 : threshold / maxAccel), fastCgazAngle);
	}

	findStopAngle(maxAccel, speedSquared, dropSpeed, dropSpeedSquared, turnCgazAngle) {
	...
	updateZone(zone, left, right, offset, zoneClass, splitZone) {
		let wrap = right > left;
	...
	updateSnaps(snapOffset, leftTarget, rightTarget) {
				...
				case 2:
					...
					if (getSize2D(MomentumPlayerAPI.GetVelocity()) > this.accelMinSpeed) {
						...
						if (left - leftTarget <= 0 && right - leftTarget >= 0) {
							stopPoint = approxEquals(zone.rightPx, zone.leftPx, 1)
								? 0
								: this.NaNCheck(
										(
											(mapAngleToScreenDist(
	...
	updatePrimeSight(viewDir, viewAngle, targetAngle, boundaryAngle, velAngle, wishDir, wishAngle) {
	...
	updateFirstPrimeZone(target, offset, zone, angles) {
	...
	applyClassBorder(panel, thickness, zoneClass) {
		panel.style.height = this.NaNCheck(zoneClass.height, 0) + 'px';
	...
	setupArrow(arrow, arrowIcon, height, width, offset, align, color) {
