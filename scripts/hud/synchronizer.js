...
const COLORS = {
	...
	SLOW: ['rgba(255, 238, 0, 1)', 'rgba(255, 255, 152, 1)'],
	NEUTRAL: ['rgba(255, 255, 255, 1)', 'rgba(178, 178, 178, 1)'],
	LOSS: ['rgba(220, 116, 13, 1)', 'rgba(255, 178, 106, 1)'],
	STOP: ['rgba(255, 106, 106, 1)', 'rgba(255, 87, 87, 1)']
};
...
class Synchronizer {
	...
	static onLoad() {
		...
		this.setSynchroMode();
		this.setSynchroColorMode();
		this.setSynchroDirection();
		...
		switch (this.mom_hud_synchro_mode) {
			case 1: // "Half-width throttle"
				...
				this.panels.segments.forEach((segment) => {
					segment.style.backgroundColor = altColor;
				});
			...
			case 2: // "Full-width throttle"
				...
				this.panels.segments.forEach((segment) => (segment.style.backgroundColor = altColor));
			...
			case 3: // "Strafe indicator"
				...
				const width = 10; // this value shows ~90% gain or better when bar touches needle
				const offset =
					this.sampleWeight *
					Math.min(
						Math.max(
				...
				this.panels.segments[0].style.width =
					(this.getBufferedSum(this.offsetHistory) * (100 - width)).toFixed(3) + '%';
				this.panels.segments[1].style.width = width + '%';
				this.panels.segments[2].style.width = 50 + '%';
				this.panels.segments[3].style.width = 50 + '%';
				this.panels.segments.forEach(
					(segment, i) => (segment.style.backgroundColor = i !== 1 ? altColor : color)
				);
			...
			case 4: // "Synchronizer"
				...
				this.panels.segments.forEach((segment, i) => {
					if (i > 0) this.panels.segments[i].style.width = this.maxSegmentWidth + '%';
					let index = i + (this.bIsFirstPanelColored ? 1 : 0);
					segment.style.backgroundColor = index % 2 ? color : altColor;
				});
	...
	static getColorTuple(ratio, bOverStrafing) {
		...
		else if (ratio > 0.98) return COLORS.PERFECT;
	 ...
	 * Sets display mode
	 */
	static setSynchroMode() {
		const newMode = GameInterfaceAPI.GetSettingFloat('mom_hud_synchro_mode');

		this.mom_hud_synchro_mode = newMode === null ? 0 : newMode;
		if (this.mom_hud_synchro_mode === 0) {
			this.panels.container.style.visibility = 'collapse';
		} else {
			this.panels.container.style.visibility = 'visible';
		}
	}
	 ...
	 * Sets colorize mode
	 */
	static setSynchroColorMode() {
		const newColorMode = GameInterfaceAPI.GetSettingFloat('mom_hud_synchro_color_enable');
	 ...
	 * Sets draw direction
	 */
	static setSynchroDirection() {
		const newDirection = GameInterfaceAPI.GetSettingFloat('mom_hud_synchro_flip_enable');

		this.mom_hud_synchro_flip_enable = newDirection === null ? 0 : newDirection;
	}

	/*
	 * Sets averaging buffer length
	 */
	static setSynchroBufferLength() {
		const newBufferLength = GameInterfaceAPI.GetSettingFloat('mom_hud_synchro_buffer_size');
