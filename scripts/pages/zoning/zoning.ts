...
class ZoneMenuHandler {
	...
	rebuildLists() {
		...
		for (const [list, panel] of Object.entries(this.panels.selectionLists)) {
			oldScrollOffset[list] = panel.GetParent()!.scrolloffset_y ?? 0;
		}
		...
		for (const [list, panel] of Object.entries(this.panels.selectionLists)) {
			...
			if (oldScrollOffset[list] <= 0) {
				scrollParent.ScrollToFitRegion(
					0,
					0,
					-oldScrollOffset[list],
					scrollParent.actuallayoutheight - oldScrollOffset[list],
