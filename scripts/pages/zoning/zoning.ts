...
class ZoneMenuHandler {
	...
	drawZones() {
		...
		for (const [segmentNumber, segment] of this.selectedZone.track.zones.segments.entries()) {
			if (segment.checkpoints.length > 0) {
				for (const [checkpointNumber, checkpoint] of segment.checkpoints.entries()) {
					for (const [_, region] of checkpoint.regions.entries()) {
						renderRegions.push({
							region: region,
							renderMode:
								checkpointNumber === 0
			...
			if (segment.cancel && segment.cancel.length > 0) {
				for (const [_, cancel] of segment.cancel.entries()) {
					for (const [_, region] of cancel.regions.entries()) {
						renderRegions.push({
							region: region,
		...
		if (this.selectedZone.track.zones.end && this.selectedZone.track.zones.end.regions.length > 0) {
			for (const [_, region] of this.selectedZone.track.zones.end.regions.entries()) {
				renderRegions.push({
					region: region,
