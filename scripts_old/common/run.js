...
class RunStats {
    ...
    constructor(runStatsObj, tickInterval, limit = -1) {
        ...
        for (let i = 1; i <= (limit === -1 ? runStatsObj.numZones : limit); i++) {
            ...
            this.zones.push(new Zone(i, (i > 1 ? this.zones[i - 2].accumulateTime : 0) + time, // Offset index by extra -1, i tracks times.values so is +1 ahead
            time, runStatsObj.statsObjects.map((stat) => new RunStat(stat.name, stat.values[i], stat.unit))));
        }
        this.overallZone = new Zone('Overall', runStatsObj.times.values[0] * tickInterval, runStatsObj.times.values[0] * tickInterval, runStatsObj.statsObjects.map((stat) => new RunStat(stat.name, stat.values[0], stat.unit)));
    }
}
class Zone {
    /**
...
class Comparison {
    ...
    constructor(baseRun, comparisonRun) {
        ...
        'Run_Comparison_Split_Overall', baseRun.stats.overallZone.accumulateTime, baseRun.stats.overallZone.accumulateTime, baseRun.stats.overallZone.accumulateTime - comparisonRun.stats.overallZone.accumulateTime, baseRun.stats.overallZone.accumulateTime - comparisonRun.stats.overallZone.accumulateTime, baseRun.stats.overallZone.stats.map((stat, j) => new RunStatsComparison(stat, comparisonRun.stats.overallZone.stats[j])));
    }
    /**
    ...
    static generateSplits(baseRunStats, comparisonRunStats) {
        return baseRunStats.zones.map((baseZone, i) => {
            ...
            return new Split(i + 1, baseZone.accumulateTime, baseZone.time, baseZone.accumulateTime - compareZone.accumulateTime, baseZone.time - compareZone.time, baseZone.stats.map((stat, j) => new RunStatsComparison(stat, compareZone.stats[j])));
        });
    }
}
class Split {
