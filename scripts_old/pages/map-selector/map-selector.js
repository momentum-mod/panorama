export const Globals = UiToolkitAPI.GetGlobalObject();
class MapSelector {
    static panels = {
        cp: $.GetContextPanel(),
    ...
    static {
        ...
        $.RegisterEventHandler('PanelLoaded', $.GetContextPanel(), () => {
            ...
            this.panels.searchText.SetPanelEvent('ontextentrychange', () => this.panels.searchClear.SetHasClass('search__clearbutton--hidden', this.panels.searchText.text === ''));
    ...
    static setupFilterSaveEvents(panel) {
        ...
        [...Globals.Util.traverseChildren(panel)]
            ...
            .forEach((panel) => panel.SetPanelEvent(this.filterablePanels[panel.paneltype].event, () => {
            ...
            storedFilters[panel.id] = {
                ...
                properties: Object.fromEntries(propertiesToStore.map((prop) => [prop, panel[prop]]))
            };
    ...
    static loadFilters() {
        ...
        const filters = $.persistentStorage.getItem('mapSelector.filtersState');
        if (!filters) {
            return;
        }
    ...
    static showConfirmOverwrite(mapID) {
        UiToolkitAPI.ShowGenericPopupOkCancel($.Localize('#Action_ConfirmOverwrite'), $.Localize('#Action_ConfirmOverwrite_Message'), 'ok-cancel-popup', () => $.DispatchEvent('MapSelector_ConfirmOverwrite', mapID), () => { });
    }
    /**
    ...
    static showConfirmCancelDownload(mapID) {
        ...
        if (GameInterfaceAPI.GetSettingBool('mom_map_download_cancel_confirm')) {
            UiToolkitAPI.ShowGenericPopupOkCancel($.Localize('#Action_ConfirmCancel'), $.Localize('#Action_ConfirmCancel_Message'), 'ok-cancel-popup', cancel, () => { });
        }
        else {
            cancel();
        }
    ...
    static onSelectedDataUpdated() {
        ...
        this.panels.websiteButton.SetPanelEvent('onactivate', () => SteamOverlayAPI.OpenURL(`https://momentum-mod.org/dashboard/maps/${mapData.id}`));
    }
    /**
