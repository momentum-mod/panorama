...
class SpeedometerDetailObject {
    ...
    saveToKV(speedometerKV) {
        ...
        if (selColorMode === SpeedometerColorTypes.RANGE)
            ...
            delete speedometerKV[SpeedometerDataKeys.RANGE_COL_PROF];
...
class Speedometers {
    ...
    static deleteSpeedometer(detailObject) {
        ...
        const idx = this.detailObjectList.findIndex((detailObjectInList) => detailObjectInList.name === detailObject.name);
        if (idx > -1) {
    ...
    static addSpeedometer() {
        UiToolkitAPI.ShowCustomLayoutPopupParameters('', 'file://{resources}/layout/modals/popups/speedometer-select.xml', `speedometerNames=${this.detailObjectList
            .map((x) => x.name)
            .join(',')}&callback=${UiToolkitAPI.RegisterJSCallback((type, name) => this.addSpeedometerByType(type, name))}`);
    }
    ...
    static reorderSpeedometer(detailObject, moveup) {
        ...
        if (moveup)
            ...
            newIdx++;
        if (newIdx < 0)
            newIdx = this.detailObjectList.length - 1;
        else if (newIdx >= this.detailObjectList.length)
            newIdx = 0;
    ...
    static resetToDefault() {
        UiToolkitAPI.ShowGenericPopupOkCancel('Reset Speedometers', 'Reset all speedometers to default? Your current speedometer config will be unrecoverable.', 'ok-cancel-popup', () => {
    ...
    static updateProfileDropdowns() {
        for (const detailObject of this.detailObjectList)
...
class RangeColorRangeDisplayObject {
    ...
    create(rangeObject, profileObject) {
        ...
        this.deleteButton.SetPanelEvent('onactivate', () => RangeColorProfiles.deleteRangeFromProfile(profileObject.name, this));
    }
    saveToKV(rangeKV) {
        const csscolor = this.displayPanel.color;
        const rangeObject = new RangeColorObject(this.displayPanel.min, this.displayPanel.max, RangeColorObject.convertCSSToKV(csscolor), csscolor);
...
class RangeColorProfileObject {
    ...
    reorderDisplayPanels() {
        if (this.displayObjectList.length === 0 || this.displayObjectList.length === 1)
            return;
        // sort on min
    ...
    createDisplayPanels(profileKV) {
        ...
        for (const displayObject of this.displayObjectList)
            displayObject?.destroy();
        this.displayObjectList = [];
        for (const rangeKV of profileKV) {
            const kvcolor = rangeKV['color'];
            const rangeObject = new RangeColorObject(rangeKV['min'], rangeKV['max'], kvcolor, RangeColorObject.convertKVToCSS(kvcolor));
            this.displayObjectList.push(new RangeColorRangeDisplayObject(rangeObject, this));
        }
        this.reorderDisplayPanels();
    }
    create(profileName, profileKV) {
        ...
        this.editButton.SetPanelEvent('onactivate', () => RangeColorProfiles.makeColorProfileNamePopup(this.name, $.Localize('#Common_Edit'), (profileName) => RangeColorProfiles.updateProfileName(this.name, profileName)));
        this.createDisplayPanels(profileKV);
    }
    markAsModified() {
        this.discardButton.enabled = true;
...
class RangeColorProfiles {
    ...
    static addProfile() {
        this.makeColorProfileNamePopup('', $.Localize('#Common_Create'), (profileName) => this.addEmptyProfile(profileName));
    }
    ...
    static updateProfileName(oldProfileName, profileName) {
        ...
        if (profileObject.name === profileName)
            return;
    ...
    static create() {
        ...
        for (const profile of this.keyvalues) {
            ...
            const profileName = profile[RangeColorProfileKeys.PROFILE_NAME];
            this.profileObjectMap.set(profileName, new RangeColorProfileObject(profileName, profile[RangeColorProfileKeys.PROFILE_RANGE_DATA]));
        }
    }
    ...
    static resetToDefault() {
        UiToolkitAPI.ShowGenericPopupOkCancel('Reset Range Color Profiles', 'Reset all range color profiles to default? Your current range color profiles config will be unrecoverable.', 'ok-cancel-popup', () => {
            // this will fire an event for loading
    ...
    static makeColorProfileNamePopup(prefilledText, OKBtnText, callback) {
        UiToolkitAPI.ShowCustomLayoutPopupParameters('', 'file://{resources}/layout/modals/popups/range-color-profile-name.xml', `profileNames=${[
            ...this.profileObjectMap.keys()
        ]}&prefilledText=${prefilledText}&OKBtnText=${OKBtnText}&callback=${UiToolkitAPI.RegisterJSCallback(callback)}`);
    }
}
