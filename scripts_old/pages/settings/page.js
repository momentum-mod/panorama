...
class SettingsShared {
    ...
    static refreshControlsRecursive(panel) {
        ...
        for (const child of panel.Children() || [])
            this.refreshControlsRecursive(child);
    }
    static resetSettingsRecursive(panel) {
        ...
        if (panel.paneltype === 'SettingsSlider' || panel.paneltype === 'SettingsEnumDropDown') {
            ...
            for (const child of panel?.Children() || [])
                this.resetSettingsRecursive(child);
        }
    }
    ...
    static showConfirmResetSettings(message, resetFn) {
        UiToolkitAPI.ShowGenericPopupTwoOptionsBgStyle($.Localize('#Settings_General_Apply'), message, 'warning-popup', $.Localize('#Action_Discard'), resetFn, $.Localize('#Action_Return'), () => { }, 'dim');
    }
    ...
    static showImportExportDialogue(localeString, panelID) {
        ...
        const findCvarsRecursive = (panel) => {
            if (panel.paneltype && this.isSettingsPanel(panel) && panel.convar)
                cvars.push(panel.convar);
            for (const child of panel?.Children() || [])
                findCvarsRecursive(child);
        };
        findCvarsRecursive(section);
        let cvarParams = '';
        for (const [i, cvar] of cvars.entries())
            cvarParams += (i !== 0 ? '&' : '') + 'cvar' + (i + 1) + '=' + cvar;
        UiToolkitAPI.ShowCustomLayoutPopupParameters('', 'file://{resources}/layout/modals/popups/import-export-settings.xml', `${cvarParams}&name=${localeString}`);
    }
    ...
    static initTextureReplacementDropdown() {
        ...
        const updatePanel = (override) => {
            ...
            if (override) {
                ...
                path = Object.entries(textures).find(([textureName, _]) => selected.text === $.Localize(textureName))[1];
            }
            ...
            if (path)
                imagePanel.SetImage(`file://{materials}/${path}.vtf`);
        };
