...
class MainMenuSettings {
        ...
        if (this.activeTab !== tab) {
            ...
            if (this.activeTab) {
                ...
                if (this.activeTab !== 'SearchSettings')
                    this.setNavItemCollapsed(this.activeTab, $.persistentStorage.getItem('settings.collapseNav') ?? true);
                // Hide the active tab
    ...
    static loadTab(tab) {
        ...
        newPanel.LoadLayout('file://{resources}/layout/pages/settings/' + _.Settings.Tabs[tab].xml + '.xml', false, false);
        ...
        if (tab !== 'SearchSettings') {
            $.RegisterEventHandler('Scroll', container, 
    ...
    static onPageScrolled(tab, panel) {
        ...
        for (const child of panel.FindChildrenWithClassTraverse('settings-group')) {
            if ((child.actualyoffset / containerHeight <= proportionScrolled &&
                proportionScrolled <=
    ...
    static setPanelInfoEvents(panel) {
        ...
        panel.SetPanelEvent('onmouseover', () => {
            ...
            panel.GetAttributeString('infotitle', '') ||
                ...
                panel.FindChildTraverse('title')?.text, message, hasDocs, panel.paneltype, panel.convar ??
                panel?.bind);
        });
    }
    static showInfo(title, message, hasDocs, paneltype, convar) {
        ...
        if (message || showConvar) {
            ...
            $.Schedule(switchDelay, () => {
                ...
                if (showConvar) {
                    this.panels.infoConvar.text = `<i>${isKeybinder ? $.Localize('#Settings_General_Command') : $.Localize('#Settings_General_Convar')}: <b>${convar}</b></i>`;
                    ...
                    this.panels.infoDocsButton.SetPanelEvent('onactivate', () => SteamOverlayAPI.OpenURLModal(`https://docs.momentum-mod.org/var/${convar}`));
                }
                else {
