"use strict";
var SettingsSearchNS;
(function (SettingsSearchNS) {
    let MatchType;
    (function (MatchType) {
        MatchType[MatchType["SETTING_TEXT"] = 0] = "SETTING_TEXT";
        MatchType[MatchType["SETTING_TAG"] = 1] = "SETTING_TAG";
        MatchType[MatchType["GROUP_TEXT"] = 2] = "GROUP_TEXT";
        MatchType[MatchType["GROUP_TAG"] = 3] = "GROUP_TAG";
    })(MatchType = SettingsSearchNS.MatchType || (SettingsSearchNS.MatchType = {}));
})(SettingsSearchNS || (SettingsSearchNS = {}));
class SettingsSearch {
    ...
    static strings = [];
    static matchedSettings = [];
    static maxMatches = 16;
    static {
    ...
    static onTextEntryChanged() {
        ...
        if (this.matchedSettings.length > 0)
            for (const matchingPanel of this.matchedSettings)
                this.createSearchResultPanel(matchingPanel);
        else {
    ...
    static searchSettingText(tabID, textPanel, tabName, groupName, groupTags) {
        ...
        const panel = MainMenuSettings.isSettingsPanel(parent) || parent.paneltype === 'ConVarEnabler'
            ? parent
            : parent.GetParent();
        ...
        for (const inputString of this.strings) {
            ...
            if (this.findMatch(inputStringMatches, inputString, $.Localize(textPanel.text), SettingsSearchNS.MatchType.SETTING_TEXT)) {
                matched = true;
            }
            ...
            if (tags)
                for (let i = 0; i < tags.length; i++)
                    if (this.findMatch(inputStringMatches, inputString, tags[i], SettingsSearchNS.MatchType.SETTING_TAG, i))
                        matched = true;
            if (groupTags)
                for (let i = 0; i < groupTags.length; i++)
                    if (this.findMatch(inputStringMatches, inputString, groupTags[i], SettingsSearchNS.MatchType.GROUP_TAG, i))
                        matched = true;
            if (!matched)
                allMatched = false;
    ...
    static findMatch(inputStringMatches, inputString, testString, type, tagIndex = 0) {
        if (!testString) {
            return false;
        }
    ...
    static shouldSearchPanelText(panel) {
        return (Object.hasOwn(panel, 'text') &&
    ...
    static createSearchResultPanel(matches) {
        ...
        if (!searchResult.LoadLayoutSnippet(isGroupPanel ? 'GroupSearchResult' : 'FullSearchResult'))
            return;
        ...
        for (const [inputString, m] of Object.entries(matches.matches ?? {}))
            for (const match of m) {
                if (match.type === SettingsSearchNS.MatchType.GROUP_TEXT)
                    groupName = groupName.replaceAll(new RegExp(`(${inputString})`, 'gi'), '<font class="settings-search-result__text--match">$1</font>');
                if (match.type === SettingsSearchNS.MatchType.SETTING_TEXT)
                    name = name.replaceAll(new RegExp(`(${inputString})`, 'gi'), '<font class="settings-search-result__text--match">$1</font>');
                if (match.type === SettingsSearchNS.MatchType.SETTING_TAG) {
                    ...
                    tags.push(matches.tags[match.tagIndex]?.replace(new RegExp(`(${inputString})`, 'gi'), '<font class="settings-search-result__tags--match">$1</font>'));
                }
                if (match.type === SettingsSearchNS.MatchType.GROUP_TAG)
                    groupTags.push(matches.groupTags[match.tagIndex]?.replace(new RegExp(`(${inputString})`, 'gi'), '<font class="settings-search-result__tags--match">$1</font>'));
            }
        searchResult.SetDialogVariable('tab_name', matches.tabName);
        searchResult.FindChild('Group').text = groupName;
        if (!isGroupPanel)
            searchResult.FindChild('Text').text = name;
        ...
        if (tags.length > 0 || groupTags.length > 0)
            ...
            tagList.AddClass('settings-search-result__tags--hidden');
