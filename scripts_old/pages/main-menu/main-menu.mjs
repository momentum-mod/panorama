import { checkDosa } from 'common/dont-show-again.mjs';
class MainMenu {
    static panels = {
        cp: $.GetContextPanel(),
    ...
    static {
        ...
        $.RegisterForUnhandledEvent('MapSelector_TryPlayMap_Outcome', (outcome) => outcome && this.onHomeButtonPressed());
    ...
    static showPlaytestWelcomePopup() {
        if (!checkDosa('playtestWelcome'))
            UiToolkitAPI.ShowCustomLayoutPopupParameters('', 'file://{resources}/layout/modals/popups/playtest-welcome.xml', 'dosaKey=playtestWelcome&dosaNameToken=Dosa_PlaytestWelcome');
    }
    /**
     * Set the video background based on persistent storage settings
     */
    static setMainMenuBackground() {
        if (!this.panels.movie?.IsValid() || !this.panels.image?.IsValid())
            return;
    ...
    static toggleBackgroundLightDark() {
        $.persistentStorage.setItem('settings.mainMenuBackground', $.persistentStorage.getItem('settings.mainMenuBackground') === 0 ? 1 : 0);
        this.setMainMenuBackground();
    }
    /**
    ...
    static onQuitButtonPressed() {
        if (GameInterfaceAPI.GetGameUIState() === 4 /* GameUIState.PAUSEMENU */) {
    ...
    static onQuitPrompt(toDesktop = true) {
        if (!toDesktop)
            return; // currently don't handle disconnect prompts
        $.DispatchEvent('MainMenuPauseGame'); // make sure game is paused so we can see the popup if hit from a keybind in-game
        UiToolkitAPI.ShowGenericPopupTwoOptionsBgStyle($.Localize('#Action_Quit'), $.Localize('#Action_Quit_Message'), 'warning-popup', $.Localize('#Action_Quit'), this.quitGame, $.Localize('#Action_Return'), () => { }, 'blur');
    }
    /** Shows a safeguard popup when disconnect is pressed during a run and safeguard is on */
    static onSafeguardDisconnect() {
        UiToolkitAPI.ShowGenericPopupOkCancel($.Localize('#Safeguard_MapQuitToMenu'), $.Localize('#Safeguard_MapQuitToMenu_Message'), 'warning-popup', () => $.DispatchEvent('Safeguard_Response', _.Safeguards.RunSafeguardType.QUIT_TO_MENU), () => { });
    }
    /** Shows a safeguard popup when quit is pressed during a run and safeguard is on */
    static onSafeguardQuit() {
        UiToolkitAPI.ShowGenericPopupOkCancel($.Localize('#Safeguard_MapQuitGame'), $.Localize('#Safeguard_MapQuitGame_Message'), 'warning-popup', () => $.DispatchEvent('Safeguard_Response', _.Safeguards.RunSafeguardType.QUIT_GAME), () => { });
    }
    /** Shows a safeguard popup when map change is triggered during a run and safeguard is on */
    static onSafeguardMapChange(mapName) {
        UiToolkitAPI.ShowGenericPopupOkCancel($.Localize('#Safeguard_MapChange'), $.Localize('#Safeguard_MapChange_Message').replace('%map%', mapName), 'warning-popup', () => GameInterfaceAPI.ConsoleCommand('__map_change_ok 1;map ' + mapName), () => { });
    }
    /** Quits the game. Bye! */
    static quitGame() {
