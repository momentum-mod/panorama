...
class EndOfRunXP {
    static xpData;
    static primaryWidth;
    static baseLevelUpTime = 3;
    static levelIndicatorMaxTransitionTime = 0.7;
    static xpCounterTicks = 100;
    static panels = {
        cp: $.GetContextPanel(),
    ...
    static onRunDataReceived(uploaded, cosXp, _rankXp, lvlGain) {
        ...
        const widthAnimation = (width, duration) => new Promise((resolve) => {
        ...
        const runXPCounter = (startXP, endXP, totalLevelXP, duration) => {
            ...
            for (let i = 0; i <= iterations; i++) {
                ...
                $.Schedule(i / this.xpCounterTicks, () => this.panels.xpCounter.SetDialogVariableInt('xp_counter', Math.round((i / iterations) * range + startXP)));
            }
        };
            ...
            if (levelDiff === 1) {
                ...
                for (let i = 1; i < levelDiff; i++)
                    chain = chain.then(() => {
                        ...
                        const duration = ((4 * this.baseLevelUpTime) / levelDiff ** 2) * (1 - 1 / phi) * (i ** 2 - levelDiff * i) +
