...
class EndOfRun {
    ...
    static updateRunStatusIndicator(status, type) {
        const statusPanel = type === _.Run.RunStatusTypes.UPLOAD ? this.panels.uploadStatus : this.panels.saveStatus;
        statusPanel.SetHasClass('spin-clockwise', status === _.Run.RunStatusStates.PROGRESS);
        statusPanel.SetHasClass('endofrun__run-status-indicator--progress', status === _.Run.RunStatusStates.PROGRESS);
        statusPanel.SetHasClass('endofrun__run-status-indicator--success', status === _.Run.RunStatusStates.SUCCESS);
        ...
        switch (status) {
            ...
            case _.Run.RunStatusStates.SUCCESS:
                icon = `file://{images}/${type === _.Run.RunStatusTypes.UPLOAD
                    ? _.Run.RunStatusIcons.UPLOAD
                    : _.Run.RunStatusIcons.SAVE}.svg`;
                text = $.Localize(type === _.Run.RunStatusTypes.UPLOAD
    ...
    static updateRunSavedStatus(saved) {
        this.updateRunStatusIndicator(saved ? _.Run.RunStatusStates.SUCCESS : _.Run.RunStatusStates.ERROR, _.Run.RunStatusTypes.SAVE);
    }
    static updateRunUploadStatus(uploaded, _cosXp, _rankXp, _lvlGain) {
        this.updateRunStatusIndicator(uploaded ? _.Run.RunStatusStates.SUCCESS : _.Run.RunStatusStates.ERROR, _.Run.RunStatusTypes.UPLOAD);
    }
    static watchReplay() {
    ...
    static setComparisionStats() {
        if (!this.baseRun || !this.comparisonRun)
            return;
        ...
        for (const [i, split] of comparison.splits.entries()) {
            ...
            if (i === comparison.splits.length - 1)
    ...
    static updateGraph(comparison, statIndex = null) {
        ...
        if (lineGraph.width === 0 || lineGraph.height === 0) {
            const setDimensionsAsSoonAsPossible = (i) => {
                ...
                $.Schedule(0.01, () => {
                    ...
                    if (lineGraph.width === 0 || lineGraph.height === 0)
                        setDimensionsAsSoonAsPossible(i);
                    else {
        ...
        if (max > 0 && max !== comparisonSplits.at(-1).diff)
            max += spacing;
        if (min < 0 && min !== comparisonSplits.at(-1).diff)
            min -= spacing;
        const range = max - min;
        ...
        let yInterval;
        if (range / scaleFactor >= 2)
            yInterval = scaleFactor;
        else if (range / scaleFactor >= 1)
            yInterval = scaleFactor / 2;
        else
            yInterval = scaleFactor / 4;
        // Set the axis
        lineGraph.axis = [
            {
                min: 0,
            ...
            },
            {
                min: min,
                max: max,
                name: $.Localize(useStat ? comparisonSplits[0].statsComparisons[statIndex].name : '#Run_Stat_Name_Time'),
                interval: yInterval,
                timeBased: !useStat
            }
        ];
        ...
        for (const [i, split] of comparisonSplits.entries()) {
            ...
            if (isTimeComparison) {
                const deltaStyle = (data.diff < 0 ? 'split--ahead ' : data.diff > 0 ? 'split--behind ' : '') +
                ...
                tooltipString =
                    '{s:name}: <b>{s:base_value}</b>\n' +
                        ...
                        `${$.Localize('#Run_Comparison_Diff')}: <b class='${diffStyle}'>${this.getDiffSign(data.diff)}{s:diff}</b>`;
            }
            line.points.push({
                ...
                events: {
                    ...
                    onmouseover: (id) => {
                        if (isTimeComparison) {
                            ...
                            this.panels.cp.SetDialogVariableInt('base_value', this.roundFloat(data.baseValue, 2));
                            this.panels.cp.SetDialogVariableInt('compare_value', this.roundFloat(data.comparisonValue, 2));
                            this.panels.cp.SetDialogVariableInt('diff', this.roundFloat(data.diff, 2));
                        }
    ...
    static setSelectedSplit(split, comparison) {
        ...
        for (const [i, statComparison] of split.statsComparisons.entries())
            createRow(statComparison, i);
    }
    /**
