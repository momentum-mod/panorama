...
class About {
    ...
    static initCreditEvents() {
        for (const panel of this.credits.FindChildrenWithClassTraverse('about-credits__name')) {
            ...
            if (!name && !roles && !bio && section === 'contributor')
                continue;
            ...
            const constructedBioString = bio
                ? Array.from({ length: Math.ceil(bio.length / maxLength) })
                    ...
                    .join('')
                : '';
            panel.SetPanelEvent('onmouseover', () => UiToolkitAPI.ShowCustomLayoutParametersTooltip(panel.id, 'CreditsTooltip', 'file://{resources}/layout/modals/tooltips/credit.xml', `username=${panel.GetAttributeString('username', '')}` +
                `&name=${name}` +
                `&section=${section}` +
                `&roles=${roles}` +
