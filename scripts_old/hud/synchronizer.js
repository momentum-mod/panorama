...
class Synchronizer {
    ...
    static onLoad() {
        ...
        if (this.statMode)
            this.onJump(); // show stats if enabled
    }
    static onUpdate() {
    ...
    static getColorPair(ratio, bOverStrafing) {
        ...
        if (!bOverStrafing) {
            if (ratio > 0.85)
                return lerpColorPairs(COLORS.SLOW, COLORS.GOOD, (ratio - 0.85) / 0.1);
            else if (ratio > 0.75)
                return COLORS.SLOW;
            else if (ratio > 0.5)
                return lerpColorPairs(COLORS.NEUTRAL, COLORS.SLOW, (ratio - 0.5) / 0.25);
            else if (ratio > 0)
                return COLORS.NEUTRAL;
            else if (ratio > -5)
                return lerpColorPairs(COLORS.NEUTRAL, COLORS.STOP, Math.abs(ratio) / 5);
        }
        else {
            if (ratio > 0.8)
                return lerpColorPairs(COLORS.SLOW, COLORS.GOOD, (ratio - 0.8) / 0.15);
            else if (ratio > 0)
                return lerpColorPairs(COLORS.LOSS, COLORS.SLOW, (ratio - 0.25) / 0.55);
            else if (ratio > -5)
                return lerpColorPairs(COLORS.LOSS, COLORS.STOP, Math.abs(ratio) / 5);
        }
    }
