...
class SpeedometerObject {
    constructor(type, speedometerPanel, settings) {
        ...
        /** @type {number} */
        this.prevVal = 0;
        this.speedometerLabel.AddClass(this.type === _.Speedo.SpeedometerTypes.OVERALL_VELOCITY ? AXIS_LABEL_CLASS : EVENT_LABEL_CLASS);
        this.comparisonLabel.AddClass(this.type === _.Speedo.SpeedometerTypes.OVERALL_VELOCITY
            ? AXIS_COMPLABEL_CLASS
            : EVENT_COMPLABEL_CLASS);
        this.comparisonLabel.SetHasClass(HIDDEN_CLASS, this.settings[_.Speedo.SpeedometerDataKeys.COLOR_TYPE] !==
            _.Speedo.SpeedometerColorTypes.COMPARISON_SEP);
...
class Speedometer {
    ...
    static registerFadeoutEventHandlers() {
        for (const [type, speedometers] of SpeedometerMap) {
            ...
            for (const speedometer of speedometers) {
                speedometer.fadeoutEventHandle = $.RegisterEventHandler('PropertyTransitionEnd', speedometer.speedometerPanel, (_, propertyName) => {
    ...
    static unregisterFadeoutEventHandlers() {
        for (const [type, speedometers] of SpeedometerMap) {
            if (!this.canSpeedometerTypeFadeOut(type))
                continue;
            for (const speedometer of speedometers) {
                if (!speedometer.fadeoutEventHandle)
                    continue;
                $.UnregisterEventHandler('PropertyTransitionEnd', speedometer.speedometerPanel, speedometer.fadeoutEventHandle);
            }
        }
    }
    ...
    static onZoneChange(enter, linear, curZone, _curTrack, timerState) {
        ...
        if (curZone <= this.lastZone && linear)
            return;
        ...
        if (linear === !enter && !exitStartOnLinear)
            return;
    ...
    static resetSpeedometerFadeouts() {
        for (const [type, speedometers] of SpeedometerMap) {
            if (!this.canSpeedometerTypeFadeOut(type))
                continue;
            for (const speedometer of speedometers)
                this.resetSpeedometerFadeout(speedometer);
        }
    }
    ...
    static getSpeedFromVelocity(velocity, settings) {
        ...
        if (numAxes > 1) {
            let squaredParts = 0;
            if (enabledAxes[0])
                squaredParts += Math.pow(velocity.x, 2);
            if (enabledAxes[1])
                squaredParts += Math.pow(velocity.y, 2);
            if (enabledAxes[2])
                squaredParts += Math.pow(velocity.z, 2);
            return Math.sqrt(squaredParts);
        }
        else if (numAxes === 1) {
            if (enabledAxes[0])
                return Math.abs(velocity.x);
            else if (enabledAxes[1])
                return Math.abs(velocity.y);
            else if (enabledAxes[2])
    ...
    static updateZoneSpeedometers(absSpeed, horizSpeed, hasComparison = true, absCustomdiff, horizCustomdiff) {
        ...
        for (const speedometer of speedometers) {
            ...
            const enabledAxes = speedometer.settings[_.Speedo.SpeedometerDataKeys.ENABLED_AXES];
            const isHoriz = enabledAxes[0] && enabledAxes[1] && !enabledAxes[2];
            this.updateSpeedometer(SpeedometerTypes.ZONE_VELOCITY, speedometer, isHoriz ? horizSpeed : absSpeed, hasComparison, isHoriz ? absCustomdiff : horizCustomdiff);
        }
    }
    static updateSpeedometersOfType(type, velocity) {
        ...
        for (const speedometer of speedometers) {
            ...
            const speed = type === _.Speedo.SpeedometerTypes.JUMP_VELOCITY
                ? velocity
    ...
    static updateSpeedometer(type, speedometer, speed, hasComparison = true, customdiff) {
        ...
        const speedometerHasComparison = colorType === _.Speedo.SpeedometerColorTypes.COMPARISON || separateComparison;
        if (hasComparison && speedometerHasComparison) {
            ...
            if (colorType === _.Speedo.SpeedometerColorTypes.RANGE && rangeList) {
                let found = false;
                ...
                // backup to white
                if (!found)
                    speedometer.speedometerLabel.style.color = 'rgba(255, 255, 255, 1)';
            }
        }
    ...
    static appendRangeColorProfileInfo(speedoData, colorProfData) {
        if (speedoData[_.Speedo.SpeedometerDataKeys.COLOR_TYPE] !== _.Speedo.SpeedometerColorTypes.RANGE)
            return;
        const colorProf = speedoData[_.Speedo.SpeedometerDataKeys.RANGE_COL_PROF];
        if (!colorProf)
            return;
        const foundProfile = colorProfData.find((profile) => colorProf === profile[_.Speedo.RangeColorProfileKeys.PROFILE_NAME]);
        if (!foundProfile)
            return;
        const rangesKV = foundProfile[_.Speedo.RangeColorProfileKeys.PROFILE_RANGE_DATA];
        if (!rangesKV)
            return;
    ...
    static onSettingsUpdate(success) {
        ...
        for (const speedoData of data) {
            const speedoType = speedoData[_.Speedo.SpeedometerDataKeys.TYPE];
            if (speedoType === undefined || speedoType === null)
                continue;
