...
(function (Cgaz_1) {
    ...
    let TruenessMode;
    (function (TruenessMode) {
        TruenessMode[TruenessMode["GROUND"] = 1] = "GROUND";
        TruenessMode[TruenessMode["PROJECTED"] = 2] = "PROJECTED";
        TruenessMode[TruenessMode["CPM_TURN"] = 4] = "CPM_TURN"; // show a/d and ground zones
    })(TruenessMode || (TruenessMode = {}));
    let InputMode;
    (function (InputMode) {
        InputMode[InputMode["NONE"] = 0] = "NONE";
        InputMode[InputMode["AIR_CONTROL"] = 1] = "AIR_CONTROL";
        InputMode[InputMode["STRAFE_LEFT"] = 2] = "STRAFE_LEFT";
        InputMode[InputMode["STRAFE_RIGHT"] = 3] = "STRAFE_RIGHT";
        InputMode[InputMode["TURN_LEFT"] = 4] = "TURN_LEFT";
        InputMode[InputMode["TURN_RIGHT"] = 5] = "TURN_RIGHT";
    })(InputMode || (InputMode = {}));
    let NEUTRAL_CLASS;
    let SLOW_CLASS;
    let FAST_CLASS;
    ...
    class Cgaz {
        ...
        static snapZones = [];
        static snapSplitZone = this.initZonePanel($.CreatePanel('Panel', this.snapContainer, 'SnapSplitZone'));
        static primeContainer = $('#PrimeContainer');
        static primeZones = [];
        static primeFirstZoneLeft = this.initZonePanel($.CreatePanel('Panel', this.primeContainer, 'PrimeFirstZoneLeft'));
        static primeFirstZoneRight = this.initZonePanel($.CreatePanel('Panel', this.primeContainer, 'PrimeFirstZoneRight'));
        static primeSplitZone = this.initZonePanel($.CreatePanel('Panel', this.primeContainer, 'PrimeSplitZone'));
        static primeHighlightZone = this.initZonePanel($.CreatePanel('Panel', this.primeContainer, 'PrimeHighlightZone'));
        ...
        static windicatorZone = this.initZonePanel($('#WindicatorZone'));
        ...
        static hudFov;
        static accelEnable;
        static accelMinSpeed;
        static accelHeight;
        static accelOffset;
        static accelSlowColor;
        static accelFastColor;
        static accelTurnColor;
        static accelDzColor;
        static accelScaleEnable;
        static accelMirrorEnable;
        static accelMirrorBorder;
        static snapEnable;
        static snapMinSpeed;
        static snapHeight;
        static snapOffset;
        static snapColor;
        static snapAltColor;
        static snapFastColor;
        static snapSlowColor;
        static snapHlColor;
        static snapHlAltColor;
        static snapHlMode;
        static snapColorMode;
        static snapHeightgainEnable;
        static primeEnable;
        static primeTruenessMode;
        static primeShowInactive;
        static primeLockOneLine;
        static primeMinSpeed;
        static primeHeight;
        static primeOffset;
        static primeGainColor;
        static primeLossColor;
        static primeAltColor;
        static primeHlEnable;
        static primeHlBorder;
        static primeHlColor;
        static primeHeightgainEnable;
        static primeColorgainEnable;
        static primeArrowEnable;
        static primeArrowSize;
        static primeArrowColor;
        static windicatorEnable;
        static windicatorHeight;
        static windicatorOffset;
        static windicatorSize;
        static windicatorColor;
        static windicatorBorder;
        static compassMode;
        static compassArrowSize;
        static compassTickSize;
        static compassOffset;
        static compassPitchEnable;
        static compassPitchTarget;
        static compassPitchWidth;
        static compassPitchOffset;
        static compassStatMode;
        static compassColor;
        static compassHlColor;
        ...
        static onLoad() {
            if (_.Web.GamemodeCategories.get(_.Web.GamemodeCategory.DEFRAG).includes(GameModeAPI.GetCurrentGameMode())) {
                this.updateHandle = $.RegisterEventHandler('HudProcessInput', $.GetContextPanel(), this.onUpdate.bind(this));
        ...
        static onPrimeConfigChange() {
            ...
            this.setupArrow(this.primeArrow, this.primeArrowIcon, height, width, this.primeOffset, 'top', this.primeArrowColor);
        }
        static onWindicatorConfigChange() {
            ...
            this.setupArrow(this.windicatorArrow, this.windicatorArrowIcon, arrowHeight, arrowWidth, this.windicatorOffset, 'top', this.windicatorColor);
        ...
        static onCompassConfigChange() {
            ...
            this.setupArrow(this.compassArrow, this.compassArrowIcon, height, width, this.compassOffset, 'bottom', this.compassColor);
        }
        static onUpdate() {
            ...
            const speed = _.Util.Maths.getSize2D(velocity);
            ...
            const velDir = _.Util.Maths.getNormal2D(velocity, 0.001);
            const velAngle = Math.atan2(velocity.y, velocity.x);
            const wishDir = lastMoveData.wishdir;
            const wishAngle = _.Util.Maths.getSizeSquared2D(wishDir) > 0.001 ? Math.atan2(wishDir.y, wishDir.x) : 0;
            const viewAngle = (MomentumPlayerAPI.GetAngles().y * Math.PI) / 180;
            const viewDir = {
                x: Math.cos(viewAngle),
                y: Math.sin(viewAngle)
            };
            const forwardMove = Math.round(_.Util.Maths.getDot2D(viewDir, wishDir));
            const rightMove = Math.round(_.Util.Maths.getCross2D(viewDir, wishDir));
            const bIsFalling = lastMoveData.moveStatus === 0;
            const bHasAirControl = phyMode && _.Util.Maths.approxEquals(wishAngle, viewAngle, 0.01) && bIsFalling;
            const bSnapShift = !_.Util.Maths.approxEquals(Math.abs(forwardMove), Math.abs(rightMove), 0.01) &&
                !(phyMode && bIsFalling);
            // find cgaz angles
            const angleOffset = _.Util.Maths.remapAngle(velAngle - wishAngle);
            ...
            if (this.accelEnable) {
                ...
                if (speed >= this.accelMinSpeed && this.accelMirrorEnable) {
                    ...
                    let mirrorOffset = _.Util.Maths.remapAngle(velAngle - viewAngle);
                    const inputAngle = _.Util.Maths.remapAngle(viewAngle - wishAngle);
                    if (_.Util.Maths.approxEquals(Math.abs(inputAngle), 0.25 * Math.PI, 0.01)) {
            ...
            if (this.snapEnable && this.snapAccel) {
                const snapOffset = _.Util.Maths.remapAngle((bSnapShift ? 0 : Math.PI * 0.25 * (rightMove > 0 ? -1 : 1)) - viewAngle);
                // draw snap zones
                if (speed >= this.snapMinSpeed) {
                    const targetOffset = _.Util.Maths.remapAngle(velAngle - viewAngle);
            ...
            if (this.primeEnable) {
                ...
                if (speed > this.primeMinSpeed) {
                    const primeMaxSpeed = this.primeTruenessMode & TruenessMode.PROJECTED
                        ? lastMoveData.wishspeed
                        : lastMoveData.maxspeed;
                    const primeMaxAccel = lastMoveData.acceleration * maxSpeed * tickInterval;
                    const primeSightSpeed = this.primeTruenessMode & TruenessMode.CPM_TURN ? primeMaxSpeed : MAX_GROUND_SPEED;
                    ...
                    const boundaryAngle = this.findStopAngle(primeSightAccel, speedSquared, dropSpeed, dropSpeedSquared, targetAngle);
                    this.updatePrimeSight(viewDir, viewAngle, targetAngle, boundaryAngle, velAngle, wishDir, wishAngle);
                    // arrow
                    if (this.primeArrowEnable) {
                        if (_.Util.Maths.getSizeSquared2D(wishDir) > 0) {
                            let arrowAngle = wishAngle -
                                Math.atan2(Math.round(this.primeAccel * wishDir.y), Math.round(this.primeAccel * wishDir.x));
                            if (Math.abs(arrowAngle) < this.hFov) {
                                ...
                                arrowAngle = _.Util.Maths.remapAngle(arrowAngle + Math.PI);
                            }
                            const leftEdge = _.Util.Maths.mapAngleToScreenDist(arrowAngle, this.hFov, this.screenX, this.scale, this.projection) - this.primeArrowSize;
            ...
            let velocityAngle = _.Util.Maths.remapAngle(viewAngle - velAngle);
            // compass
            if (this.compassMode) {
                const ticks = this.tickContainer.Children();
                const bShouldHighlight = Math.abs(_.Util.Maths.remapAngle(8 * velAngle) * 0.125) < 0.01 && speed >= this.accelMinSpeed;
                const color = bShouldHighlight ? this.compassHlColor : this.compassColor;
                // ticks
                for (const [i, tick] of ticks.entries()) {
                    const tickAngle = this.NaNCheck(_.Util.Maths.wrapToHalfPi(viewAngle + i * 0.25 * Math.PI), 0);
                    const tickPx = this.NaNCheck(_.Util.Maths.mapAngleToScreenDist(tickAngle, this.hFov, this.screenX, this.scale, this.projection), 0);
                ...
                if (Math.abs(velocityAngle) < this.hFov) {
                    ...
                    this.compassArrowIcon.RemoveClass('arrow__up');
                    this.compassArrowIcon.AddClass('arrow__down');
                    velocityAngle = _.Util.Maths.remapAngle(velocityAngle - Math.PI);
                }
                const leftEdge = _.Util.Maths.mapAngleToScreenDist(velocityAngle, this.hFov, this.screenX, this.scale, this.projection) - this.compassArrowSize;
                this.compassArrow.style.position = `${this.NaNCheck(leftEdge, 0)}px 0px 0px`;
                this.compassArrowIcon.style.washColor = _.Util.Color.rgbaStringToRgb(color);
            }
            ...
            if (this.compassPitchEnable) {
                ...
                for (let i = 0; i < pitchLines?.length; ++i) {
                    ...
                    const pitchDeltaPx = _.Util.Maths.mapAngleToScreenDist((pitchDelta * Math.PI) / 180, this.vFov, this.screenY, this.scale, this.projection);
            ...
            if (this.compassStatMode) {
                ...
                this.yawStat.style.color =
                    Math.abs(_.Util.Maths.remapAngle(8 * velAngle) * 0.125) < 0.01 && speed >= this.accelMinSpeed
                        ? this.compassHlColor
                        : this.compassColor;
                ...
                for (const target of this.compassPitchTarget) {
                    if (Math.abs(viewPitch - target) <= 0.15)
            ...
            if (this.windicatorEnable && Math.abs(wTurnAngle) < this.hFov && speed >= this.accelMinSpeed) {
                ...
                const leftEdge = _.Util.Maths.mapAngleToScreenDist(wTurnAngle, this.hFov, this.screenX, this.scale, this.projection) - this.windicatorSize;
                ...
                if (bHasAirControl) {
                    // TODO: Peen - this is missing the 6th arg, dunno what it should be. Passing object so it compiles, probs wrong.
        ...
        static findSnapAngles(snapAccel) {
            ...
            for (let i = 0; i < points; ++i)
                angles.push(-breakPoints[i], _.Util.Maths.remapAngle(Math.PI * 0.5 - breakPoints[i]), _.Util.Maths.remapAngle(breakPoints[i] - Math.PI * 0.5));
        ...
        static updateZone(zone, left, right, offset, zoneClass, splitZone) {
            let wrap = right > left;
            zone.leftAngle = _.Util.Maths.remapAngle(left - offset);
            zone.rightAngle = _.Util.Maths.remapAngle(right - offset);
            wrap = zone.rightAngle > zone.leftAngle ? !wrap : wrap;
            // map angles to screen
            zone.leftPx = _.Util.Maths.mapAngleToScreenDist(zone.leftAngle, this.hFov, this.screenX, this.scale, this.projection);
            zone.rightPx = _.Util.Maths.mapAngleToScreenDist(zone.rightAngle, this.hFov, this.screenX, this.scale, this.projection);
            if (wrap) {
                ...
                splitZone.leftPx = _.Util.Maths.mapAngleToScreenDist(this.accelSplitZone.leftAngle, this.hFov, this.screenX, this.scale, this.projection);
                splitZone.rightPx = _.Util.Maths.mapAngleToScreenDist(this.accelSplitZone.rightAngle, this.hFov, this.screenX, this.scale, this.projection);
        ...
        static updateSnaps(snapOffset, leftTarget, rightTarget) {
            ...
            for (const [i, zone] of zones.entries()) {
                ...
                const left = _.Util.Maths.wrapToHalfPi(this.snapAngles[i] - snapOffset);
                const right = _.Util.Maths.wrapToHalfPi(this.snapAngles[(i + 1) % zones.length] - snapOffset);
                ...
                if (this.snapColorMode) {
                    snapColor = _.Util.Color.rgbaStringLerp(this.snapSlowColor, this.snapFastColor, alpha);
                }
                ...
                switch (this.snapHlMode) {
                    ...
                    case 2:
                        ...
                        if (_.Util.Maths.getSize2D(MomentumPlayerAPI.GetVelocity()) > this.accelMinSpeed) {
                            let stopPoint, direction;
                            if (left - leftTarget <= 0 && right - leftTarget >= 0) {
                                stopPoint = _.Util.Maths.approxEquals(zone.rightPx, zone.leftPx, 1)
                                    ? 0
                                    : this.NaNCheck(((_.Util.Maths.mapAngleToScreenDist(leftTarget, this.hFov, this.screenX, this.scale, this.projection) -
                                        zone.leftPx) /
                                        (zone.rightPx - zone.leftPx)).toFixed(3), 0);
                                direction = '0% 0%, 100% 0%';
                                bHighlight = true;
                            }
                                ...
                                stopPoint = _.Util.Maths.approxEquals(zone.rightPx, zone.leftPx, 1)
                                    ? 0
                                    : this.NaNCheck(((zone.rightPx -
                                        _.Util.Maths.mapAngleToScreenDist(rightTarget, this.hFov, this.screenX, this.scale, this.projection)) /
                                        (zone.rightPx - zone.leftPx)).toFixed(3), 0);
                                direction = '100% 0%, 0% 0%';
                                bHighlight = true;
                            }
        ...
        static updatePrimeSight(viewDir, viewAngle, targetAngle, boundaryAngle, velAngle, wishDir, wishAngle) {
            const cross = _.Util.Maths.getCross2D(wishDir, viewDir);
            const inputMode = Math.round(_.Util.Maths.getSize2D(wishDir)) * (1 << Math.round(2 * Math.pow(cross, 2))) +
                (Math.round(cross) > 0 ? 1 : 0);
            const angleOffset = _.Util.Maths.remapAngle(velAngle - wishAngle);
            const targetOffset = _.Util.Maths.remapAngle(velAngle - viewAngle);
            const inputAngle = _.Util.Maths.remapAngle(viewAngle - wishAngle) * _.Util.Maths.getSizeSquared2D(wishDir);
            const velocity = MomentumPlayerAPI.GetVelocity();
            ...
            const leftTarget = _.Util.Maths.wrapToHalfPi(-targetAngle - velAngle);
            const leftAngles = fillLeftZones ? this.primeAngles : this.snapAngles;
            const rightOffset = Math.PI * 0.25 - viewAngle;
            const rightTarget = _.Util.Maths.wrapToHalfPi(targetAngle - velAngle);
            ...
            if (fillLeftZones || this.primeShowInactive) {
                this.primeFirstZoneLeft.rightPx = _.Util.Maths.mapAngleToScreenDist(_.Util.Maths.wrapToHalfPi(leftTarget - leftOffset), this.hFov, this.screenX, this.scale, this.projection);
            ...
            speedGain = this.findPrimeGain(this.primeFirstZoneLeft, velocity, _.Util.Maths.rotateVector2D(viewDir, 0.25 * Math.PI));
            gainZonesMap.set(this.primeFirstZoneLeft, speedGain);
            if (speedGain > gainMax)
                gainMax = speedGain;
            if (fillRightZones || this.primeShowInactive) {
                this.primeFirstZoneRight.leftPx = _.Util.Maths.mapAngleToScreenDist(_.Util.Maths.wrapToHalfPi(rightTarget - rightOffset), this.hFov, this.screenX, this.scale, this.projection);
            ...
            speedGain = this.findPrimeGain(this.primeFirstZoneRight, velocity, _.Util.Maths.rotateVector2D(viewDir, -0.25 * Math.PI));
            gainZonesMap.set(this.primeFirstZoneRight, speedGain);
            if (speedGain > gainMax)
                gainMax = speedGain;
            if (fillLeftZones) {
                const leftBoundary = _.Util.Maths.wrapToHalfPi(-boundaryAngle - velAngle);
            ...
            if (fillRightZones) {
                const rightBoundary = _.Util.Maths.wrapToHalfPi(boundaryAngle - velAngle);
            ...
            for (const [zone, gain] of gainZonesMap.entries()) {
                ...
                if (gain < 0) {
                    ...
                    zone.color = _.Util.Color.rgbaStringLerp(this.primeAltColor, this.primeGainColor, gainFactor);
                }
                else {
                ...
                if (this.primeHlEnable && this.shouldHighlight(zone)) {
                    ...
                    zone.color = _.Util.Color.enhanceAlpha(zone.color);
                }
        ...
        static fillActivePrimeZones(zoneRange, offset, gainZonesMap, gainMax, velocity, wishDir) {
            ...
            while (count-- >= 0) {
                ...
                const left = _.Util.Maths.wrapToHalfPi(this.primeAngles[index] - offset);
                const right = _.Util.Maths.wrapToHalfPi(this.primeAngles[(index + 1) % angleCount] - offset);
        ...
        static findArrayInfimum(arr, val) {
            ...
            if (val < arr[lower] || val > arr[upper])
                return upper;
            while (lower < upper) {
        ...
        static findPrimeGain(zone, velocity, wishDir) {
            ...
            const zoneVector = _.Util.Maths.rotateVector2D(wishDir, -avgAngle);
            const snapProject = {
                x: Math.round(zoneVector.x * this.primeAccel),
                y: Math.round(zoneVector.y * this.primeAccel)
            };
            const newSpeed = _.Util.Maths.getSize2D({
                x: Number(velocity.x) + Number(snapProject.x),
                y: Number(velocity.y) + Number(snapProject.y)
            });
            return newSpeed - _.Util.Maths.getSize2D(velocity);
        }
        static updateFirstPrimeZone(target, offset, zone, angles) {
            const i = this.findArrayInfimum(angles, target);
            const left = _.Util.Maths.wrapToHalfPi(angles[i] - offset);
            const right = _.Util.Maths.wrapToHalfPi(angles[(i + 1) % angles.length] - offset);
            this.updateZone(zone, left, right, 0, PRIME_SIGHT_CLASS, this.primeSplitZone);
            return i;
        }
        ...
        static setupArrow(arrow, arrowIcon, height, width, offset, align, color) {
            ...
            arrowIcon.style.washColor = _.Util.Color.rgbaStringToRgb(color);
