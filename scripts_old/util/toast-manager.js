...
(function (Toasts) {
    ...
    // TODO: very half-assed effort porting to TS
    class Toast {
        style;
        id;
        parameters;
        ...
        isIdenticalTo(toast) {
            return ((this.id && this.id === toast.id) ||
                (this.customLayout === toast.customLayout &&
                    ...
                    (!this.parameters || !toast.parameters || _.Util.compareDeep(this.parameters, toast.parameters))));
        }
    ...
    class ToastManager {
        ...
        static {
            ...
            for (const loc of Locations)
                this.containers[loc] = $('#' + loc[0].toUpperCase() + loc.slice(1));
            $.RegisterForUnhandledEvent('Toast_Show', (id, title, message, locationNum, duration, icon, style) => this.queueToast(Toast.createToast({
                id: id,
                title: title,
                message: message,
            ...
            $.RegisterForUnhandledEvent('Toast_ShowCustom', (id, layoutFile, locationNum, duration, parameters) => this.queueToast(Toast.createToast({
                id: id,
        ...
        static deleteToastByID(toastID) {
            if (!toastID)
                return;
            for (const loc of Locations)
                this.deleteToast(this.activeToasts[loc].find((t) => t.id === toastID));
        }
