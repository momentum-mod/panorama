...
class ImportExportSettings {
    ...
    static onLoad() {
        ...
        cp.SetDialogVariable('title', $.Localize('#Settings_ImportExport_Title').replace('%thing%', $.Localize(sectionName)));
    ...
    static importSettings() {
        ...
        try {
            ...
            if (this.base64Mode) {
                if (Object.keys(this.cvars).length !== cvars.length)
                    throw '#Settings_ImportExport_Error_Outdated';
                for (const [index, cvarValue] of cvars.entries())
                    GameInterfaceAPI.SetSettingString(this.cvars[index], cvarValue);
            }
            else {
                if (!cvars[0].includes(' '))
                    throw '#Settings_ImportExport_Error_NoCvars';
                for (const cvar of cvars) {
                    const cvarArray = cvar.split(' ');
                    if (!Object.values(this.cvars).includes(cvarArray[0])) {
                        foundInvalidInput = true;
                        if (!cvar.startsWith('['))
                            this.panels.import.text = this.panels.import.text.replace(cvar, `[${$.Localize('#Settings_ImportExport_Invalid')}] ${cvar}`);
                        continue;
                    }
            ...
            $.GetContextPanel().SetDialogVariable('import_warning', $.Localize('#Settings_ImportExport_Success') +
            ...
            $.GetContextPanel().SetDialogVariable('import_warning', $.Localize('#Settings_ImportExport_Failure') + ' ' + $.Localize(error));
