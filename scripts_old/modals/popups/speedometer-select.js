...
class SpeedometerSelectPopup {
    ...
    static validateSpeedometerNames() {
        ...
        if (text === '' || text.includes(','))
            return false;
        if (this.speedometerNames.some((name) => text.toUpperCase() === name.toUpperCase()))
            return false;
        return true;
    }
    ...
    static init() {
        ...
        for (const typeNum of _.Functions.Enum.values(_.PanoConst.SpeedometerTypes)) {
            ...
            speedometer.FindChildInLayoutFile('SpeedometerBtnLabel').text = $.Localize(_.PanoConst.SpeedometerDispNames.get(typeNum));
            ...
            if (typeNum === SpeedometerTypes.OVERALL_VELOCITY)
