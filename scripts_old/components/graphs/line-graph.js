...
(function (LineGraph) {
    class Component {
        ...
        static draw() {
            ...
            if (!this.axis.every((axis) => !Number.isNaN(axis.min) && !Number.isNaN(axis.max)) ||
                ...
                for (let j = lineMin; j <= lineMax; j += axis.interval) {
                    ...
                    if (dist !== 0 && dist !== panelLength)
                        $.CreatePanel('Panel', this.panels.grid, '', {
                            ...
                            class: 'linegraph__gridline linegraph__gridline--' +
                                axisName +
            ...
            for (const [lineIndex, line] of this.lines.entries()) {
                ...
                if (line.shadeAboveToOriginColor || line.shadeBelowToOriginColor) {
                    ...
                    const findPoly = (isAbove, trackingBool) => {
                        for (const [i, point] of line.points.entries()) {
                            if (isAbove ? point.y > 0 : point.y < 0) {
                                ...
                                polyPoints.push({ x: lastPoint.x, y: 0 }, { x: lastPoint.x, y: lastPoint.y }, { x: xIntersect, y: 0 });
                                drawPoly(isAbove);
                            }
                        }
                    };
                    ...
                    if (line.shadeAboveToOriginColor)
                        findPoly(true, isDoingAbovePoly);
                    if (line.shadeBelowToOriginColor)
                        findPoly(false, isDoingBelowPoly);
                }
            }
        }
        /**
