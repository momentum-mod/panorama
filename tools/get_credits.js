...
const PATH =
    process.env.npm_package_config_baseDir + "layout/drawer/credits.xml";
...
(async () => {
    try {
        const makeSection = (sectionArray, columns) => {
            ...
            xmlString.openPanel(
                "Label",
                { class: `${CREDITS_CLASS}__subheader`, text: sectionName },
                true
            );
        ...
        Object.values(TYPES).forEach((type) => {
            xmlString.openPanel(
                "Label",
                { class: `${CREDITS_CLASS}__header`, text: type.title },
                true
            );
            ...
            [
                ...new Set(
                    data
                        .filter((person) => person["Type"] === type.sheetName)
